<?php
	...
	*/
	...
	function get_sql_fields($table_name) {
		$sql_fields = [
			'Clients' => "`Clients`.`id` as 'id', `Clients`.`name` as 'name', `Clients`.`logo` as 'logo', `Clients`.`endpoints` as 'endpoints', `Clients`.`jobs` as 'jobs', `Clients`.`destinations` as 'destinations'",
			'Endpoints' => "`Endpoints`.`id` as 'id', `Endpoints`.`hostname` as 'hostname', `Endpoints`.`notes` as 'notes', IF(    CHAR_LENGTH(`Clients1`.`name`), CONCAT_WS('',   `Clients1`.`name`), '') as 'clientid'",
			'Jobs' => "`Jobs`.`id` as 'id', IF(    CHAR_LENGTH(`Clients1`.`name`), CONCAT_WS('',   `Clients1`.`name`), '') as 'client', IF(    CHAR_LENGTH(`Software1`.`name`), CONCAT_WS('',   `Software1`.`name`), '') as 'software', IF(    CHAR_LENGTH(`Endpoints1`.`hostname`), CONCAT_WS('',   `Endpoints1`.`hostname`), '') as 'endpoint', IF(    CHAR_LENGTH(`Methods1`.`name`) || CHAR_LENGTH(`Software2`.`name`), CONCAT_WS('',   `Methods1`.`name`, ' - ', `Software2`.`name`), '') as 'method', IF(    CHAR_LENGTH(`Destinations1`.`type`) || CHAR_LENGTH(`Destinations1`.`id`), CONCAT_WS('',   `Destinations1`.`type`, '- ', `Destinations1`.`id`), '') as 'destination'",
			'Log' => "`Log`.`id` as 'id', if(`Log`.`date`,date_format(`Log`.`date`,'%d-%m-%Y'),'') as 'date', `Log`.`user` as 'user', IF(    CHAR_LENGTH(`Clients1`.`name`), CONCAT_WS('',   `Clients1`.`name`), '') as 'client', IF(    CHAR_LENGTH(`Endpoints1`.`hostname`), CONCAT_WS('',   `Endpoints1`.`hostname`), '') as 'endpoint', IF(    CHAR_LENGTH(`Endpoints2`.`hostname`) || CHAR_LENGTH(`Destinations1`.`type`) || CHAR_LENGTH(`Destinations1`.`id`), CONCAT_WS('',   `Endpoints2`.`hostname`, `Destinations1`.`type`, '- ', `Destinations1`.`id`), '') as 'job', `Log`.`status` as 'status', `Log`.`results` as 'results', `Log`.`screenshot` as 'screenshot', `Log`.`notes` as 'notes'",
			'Methods' => "`Methods`.`id` as 'id', `Methods`.`name` as 'name', IF(    CHAR_LENGTH(`Software1`.`name`), CONCAT_WS('',   `Software1`.`name`), '') as 'software', `Methods`.`procedure` as 'procedure'",
			'Software' => "`Software`.`id` as 'id', `Software`.`name` as 'name', `Software`.`notes` as 'notes'",
			'Destinations' => "`Destinations`.`id` as 'id', `Destinations`.`hostname` as 'hostname', `Destinations`.`type` as 'type', IF(    CHAR_LENGTH(`Clients1`.`name`), CONCAT_WS('',   `Clients1`.`name`), '') as 'client', `Destinations`.`space` as 'space'",
		];
	...
	function get_sql_from($table_name, $skip_permissions = false, $skip_joins = false, $lower_permissions = false) {
		$sql_from = [
			'Clients' => "`Clients` ",
			'Endpoints' => "`Endpoints` LEFT JOIN `Clients` as Clients1 ON `Clients1`.`id`=`Endpoints`.`clientid` ",
			'Jobs' => "`Jobs` LEFT JOIN `Clients` as Clients1 ON `Clients1`.`id`=`Jobs`.`client` LEFT JOIN `Software` as Software1 ON `Software1`.`id`=`Jobs`.`software` LEFT JOIN `Endpoints` as Endpoints1 ON `Endpoints1`.`id`=`Jobs`.`endpoint` LEFT JOIN `Methods` as Methods1 ON `Methods1`.`id`=`Jobs`.`method` LEFT JOIN `Software` as Software2 ON `Software2`.`id`=`Methods1`.`software` LEFT JOIN `Destinations` as Destinations1 ON `Destinations1`.`id`=`Jobs`.`destination` ",
			'Log' => "`Log` LEFT JOIN `Clients` as Clients1 ON `Clients1`.`id`=`Log`.`client` LEFT JOIN `Endpoints` as Endpoints1 ON `Endpoints1`.`id`=`Log`.`endpoint` LEFT JOIN `Jobs` as Jobs1 ON `Jobs1`.`id`=`Log`.`job` LEFT JOIN `Endpoints` as Endpoints2 ON `Endpoints2`.`id`=`Jobs1`.`endpoint` LEFT JOIN `Destinations` as Destinations1 ON `Destinations1`.`id`=`Jobs1`.`destination` ",
			'Methods' => "`Methods` LEFT JOIN `Software` as Software1 ON `Software1`.`id`=`Methods`.`software` ",
			'Software' => "`Software` ",
			'Destinations' => "`Destinations` LEFT JOIN `Clients` as Clients1 ON `Clients1`.`id`=`Destinations`.`client` ",
		];
