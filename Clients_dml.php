	...
	$data = [
		...
		'logo' => Request::fileUpload('logo', [
			...
			'failure' => function($selected_id, $fileRemoved) {
				...
				return existing_value('Clients', 'logo', Request::val('SelectedID'));
			},
		]),
	];
...
function Clients_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	...
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `Endpoints` WHERE `clientid`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
		$RetMsg = str_replace('<TableName>', 'Endpoints', $RetMsg);
		return $RetMsg;
	...
	$res = sql("SELECT `id` FROM `Clients` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `Jobs` WHERE `client`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	$res = sql("SELECT `id` FROM `Clients` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `Log` WHERE `client`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	// child table: Destinations
	$res = sql("SELECT `id` FROM `Clients` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `Destinations` WHERE `client`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
		$RetMsg = str_replace('<TableName>', 'Destinations', $RetMsg);
		return $RetMsg;
	} elseif($rirow[0] && $AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation['confirm delete'];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	$data = [
		...
		'logo' => Request::fileUpload('logo', [
			...
			'failure' => function($selected_id, $fileRemoved) {
				...
				return existing_value('Clients', 'logo', $selected_id);
			},
		]),
	];
...
function Clients_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $separateDV = 0, $TemplateDV = '', $TemplateDVP = '') {
	...
	$templateCode = str_replace('<%%UPLOADFILE(endpoints)%%>', '', $templateCode);
	$templateCode = str_replace('<%%UPLOADFILE(jobs)%%>', '', $templateCode);
	$templateCode = str_replace('<%%UPLOADFILE(destinations)%%>', '', $templateCode);
	...
	if($selected_id) {
		...
		$templateCode = str_replace('<%%VALUE(endpoints)%%>', safe_html($urow['endpoints']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(endpoints)%%>', urlencode($urow['endpoints']), $templateCode);
		$templateCode = str_replace('<%%VALUE(jobs)%%>', safe_html($urow['jobs']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(jobs)%%>', urlencode($urow['jobs']), $templateCode);
		$templateCode = str_replace('<%%VALUE(destinations)%%>', safe_html($urow['destinations']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(destinations)%%>', urlencode($urow['destinations']), $templateCode);
	} else {
		...
		$templateCode = str_replace('<%%VALUE(endpoints)%%>', '', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(endpoints)%%>', urlencode(''), $templateCode);
		$templateCode = str_replace('<%%VALUE(jobs)%%>', '', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(jobs)%%>', urlencode(''), $templateCode);
		$templateCode = str_replace('<%%VALUE(destinations)%%>', '', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(destinations)%%>', urlencode(''), $templateCode);
	}
