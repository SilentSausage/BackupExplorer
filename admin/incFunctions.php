	...
	*/
		...
		$all_tables = [
			/* ['table_name' => [table props assoc array] */   
				...
				'Software' => [
					'Caption' => 'Software',
					'Description' => '',
					'tableIcon' => 'resources/table_icons/cog_edit.png',
					'group' => $tg[1],
					'homepageShowCount' => 0
				],
				'Destinations' => [
	...
	function getTableList($skip_authentication = false) {
		...
		$arrTables = [
			...
			'Clients' => ['Clients', '', 'resources/table_icons/ceo.png', 'None'],
			'Endpoints' => ['Endpoints', '', 'resources/table_icons/dns_setting.png', 'Config'],
			'Jobs' => ['Jobs', '', 'resources/table_icons/cog.png', 'Config'],
			'Log' => ['Log', '', 'resources/table_icons/application_edit.png', 'Config'],
			'Methods' => ['Methods', '', 'resources/table_icons/align_left.png', 'Config'],
			'Software' => ['Software', '', 'resources/table_icons/cog_edit.png', 'Config'],
			'Destinations' => ['Destinations', '', 'resources/table_icons/arrow_branch.png', 'Config'],
		];
	...
	function getThumbnailSpecs($tableName, $fieldName, $view) {
		if($tableName=='Clients' && $fieldName=='logo' && $view=='tv')
			return ['width'=>50, 'height'=>50, 'identifier'=>'_tv'];
		elseif($tableName=='Clients' && $fieldName=='logo' && $view=='dv')
			return ['width'=>250, 'height'=>250, 'identifier'=>'_dv'];
		elseif($tableName=='Log' && $fieldName=='screenshot' && $view=='tv')
			return ['width'=>800, 'height'=>600, 'identifier'=>'_tv'];
		return FALSE;
	}
	########################################################################
			...
			$schema = [
				'Clients' => [
					...
					'logo' => [
						...
						'info' => [
							'caption' => 'Logo',
							'description' => 'Maximum file size allowed: 3000 KB.<br>Allowed file types: jpg, jpeg, gif, png, webp',
						],
					],
					'endpoints' => [
						'appgini' => "VARCHAR(40) NULL",
	...
	function lookupQuery($tn, $lookupField) {
		...
		*/
		$lookupQuery = [
			...
			'Endpoints' => [
				'clientid' => 'SELECT `Clients`.`id`, `Clients`.`name` FROM `Clients` ORDER BY 2',
			],
			'Jobs' => [
